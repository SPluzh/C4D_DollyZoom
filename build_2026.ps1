...
Write-Host "Running CMake configure for SDK 2026..."
& $cmake -B sdk_2026/build -S sdk_2026 -G "Visual Studio 17 2022" -A x64

if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "Running MSBuild (CMake build)..."
& $cmake --build sdk_2026/build --config Release

if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$xdl64Path = "sdk_2026\build\bin\Release\plugins\C4D_DollyZoom\C4D_DollyZoom.xdl64"
if (Test-Path $xdl64Path) {
    Write-Host "Plugin build successful: $xdl64Path"
    $pdbPath = "sdk_2026\build\bin\Release\plugins\C4D_DollyZoom\C4D_DollyZoom.pdb"
